...
/**
 * 
 * @author Implementation class of the server interface
 */
public class ServerClinicImpl extends ClinicServicePOA {
	 ...
	 * @param portNum
	 */
	protected ServerClinicImpl(int portNum) {
		super();
	...
	public String createDRecord(String firstName, String lastName,
		...
		dr.setAddressMethod(address);
		dr.setSpecializationMethod(specialization);
		dr.setLocationMethod(location);
	...
	public String createNRecord(String firstName, String lastName,
		...
		// set the details received from client
		NRecord nr = new NRecord();
		int id = getNRId();
		nr.setRecordID(record + id);
		nr.setFirstName(firstName);
		nr.setLastName(lastName);
		nr.setDesignationMethod(designation);
		nr.setStatusMethod(statusDate);
		nr.setStatusDateMethod(statusDate);
				...
				fieldName.equalsIgnoreCase("designation")||fieldName.equalsIgnoreCase("status")||fieldName.equalsIgnoreCase("statusDate"))
		{
			...
			for (Map.Entry<String,List<Practitioner>> entry : practitionerRecords.entrySet())
		    {
			    ...
			    synchronized(practitionerList) 
			    {
			    	...
			    	{
			    		if(recordID.startsWith("DR")&& practitioner instanceof DRecord)
			    		{
			    			practitionerUpdate=practitioner;
			    			if(recordID.equals(practitioner.getRecordID()))
			    			{
			    				if(fieldName.equalsIgnoreCase("address")){((DRecord)practitioner).setAddressMethod(newValue); recordFound=true; break;}
			    				if(fieldName.equalsIgnoreCase("phone")){((DRecord)practitioner).setPhoneMethod(newValue);recordFound=true; break;}
			    				if(fieldName.equalsIgnoreCase("location")){((DRecord)practitioner).setLocationMethod(newValue);recordFound=true; break;}
			    			}
			    		}
			    		else if(recordID.startsWith("NR")&& practitioner instanceof NRecord)
			    		{
			    			practitionerUpdate=practitioner;
			    			if(recordID.equals(practitioner.getRecordID()))
			    			{
			    				if(fieldName.equalsIgnoreCase("designation")){((NRecord)practitioner).setDesignationMethod(newValue);recordFound=true; break;}
			    				if(fieldName.equalsIgnoreCase("status")){((NRecord)practitioner).setStatusMethod(newValue);recordFound=true; break;}
			    				if(fieldName.equalsIgnoreCase("statusDate")){((NRecord)practitioner).setStatusDateMethod(newValue);recordFound=true; break;}
			    			}
			    		}
			    	}
		        }
			...
			if(practitionerUpdate instanceof DRecord)
				result = "SUCCESS: Doctor Record updated";
			if(practitionerUpdate instanceof NRecord)
	...
	private String constructRecord(Practitioner r, String managerId) {
		...
		if (recordID.contains("DR")) {
			DRecord dr = (DRecord) r;
			record.append(dr.getAddressMethod()).append("|").append(dr.getPhoneMethod()).append("|").append(dr.getSpecializationMethod()).append("|").append(dr.getLocationMethod());
		}
		if (recordID.contains("NR")) {
			NRecord nr = (NRecord) r;
			record.append(nr.getDesignationMethod()).append("|").append(nr.getStatusMethod()).append("|").append(nr.getStatusDateMethod());
		}
